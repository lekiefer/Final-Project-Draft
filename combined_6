...
vector<Vector2f>& treeCoords, vector<Vector2f>& pondCoords) {
    ...
    int intersect;
    int ranPondNum = rand() % 3 + 1;   //create random num of ponds between 1-3
    int ranTreeNum = rand() % 10 + 1;   //create random num of tree trunks between 1-10
...
void intro_text(Font font)
{
    window.clear(Color::White);
    Text text("Welcome to our game! \n \n Try to gobble the yellow 'stars' using the arrows \n before the hunters catch you \n Press 'P' to play or 'Q' to quit", font, 32);
...
int main()
{
    ...
    if (!crash.openFromFile("crash1.wav"))
        return EXIT_FAILURE;
    if (!splash.openFromFile("water2.wav"))
        return EXIT_FAILURE;
