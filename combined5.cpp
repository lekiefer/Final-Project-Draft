        ...
        for (j=0; j<Hunt_Num; j++)//finds the distance between two hunters
        {
            ...
            else if (collide_once==0)//only check the current hunter if no collision has occurred
            {
                ...
                if (dist<=rad)
                {
                    ...
                    {
                        hunters[i].setSpeed(hunters[i].getSpeed()+(hunters[j].getSpeed()/10));
                        hunters[i].setDamage(hunters[i].getDamage()+(hunters[j].getDamage()/10));
                        hunters[i].setAccuracy(hunters[i].getAccuracy()+(hunters[j].getAccuracy()*10));
                        hunters[i].setRadius(hunters[i].getRadius()+(hunters[j].getRadius()/20));
...
//        if (hunters[i].getFillColor()==sf::Color::Blue)
//        {
//            target_xy=hunters[blue_target].getPosition();
//        }
        if(counter==0)
        {
            run_toggle[i]="chase";
...
//        return EXIT_FAILURE;
    ...
    while (window.isOpen())
    {
        ...
        sf::Text text; text.setFont(font); text.setString("GAME OVER.");text.setCharacterSize(100);text.setPosition(300,200);
