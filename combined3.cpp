...
const int screenWid = 1000;
const int screenHei = 600;
...
void user_moves(sf::CircleShape& user_blip, int user_x_speed[2], int user_y_speed[2],
int& pondColl, int& treeColl) {
    char lastKeyPress = 'Z';
    char twoKeysAgo = 'Z';
...
int& pondColl, int& treeColl, int& starColl, Music& music) {
...
int main()
{
    ...
    Music music;
    if (!music.openFromFile("bell.wav"))
        return EXIT_FAILURE;
    ...
    while (user_health>0)
    {
        ...
        user_moves(user_blip,user_x_speed,user_y_speed, pondColl, treeColl);
        draw_statics();
        ...
        checkCollision(treeCoords, pondCoords, starCoords, starGroup, user_blip, isCollision, pondColl, treeColl, starColl, music);
        window.display();
    }
    Text text; text.setFont(font); text.setString("GAME OVER.");text.setCharacterSize(100);
